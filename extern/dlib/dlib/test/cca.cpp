...
// ----------------------------------------------------------------------------------------
    ...
    void test_cca3() {
...

        {
            ...
            const double corr_error = max(
                    abs(compute_correlations(rm_zeros(L * Ltrans), rm_zeros(R * Rtrans)) - correlations));
        ...
        }
        {
            ...
            DLIB_TEST(Ltrans.nc() == Rtrans.nc());
            dlog << LINFO << "correlations: " << trans(correlations);
            dlog << LINFO << "computed cors: "
                 << trans(compute_correlations(rm_zeros(L * Ltrans), rm_zeros(R * Rtrans)));
    ...
    void test_cca2() {
...

        {
            ...
            if (Ltrans.nc() > 1) {
                ...
                const double corr_rot1_error = max(
                        abs(compute_correlations(rm_zeros(L * rotate<0, 1>(Ltrans)), rm_zeros(R * Rtrans))));
