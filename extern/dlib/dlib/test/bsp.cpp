...
// ----------------------------------------------------------------------------------------
    ...
    template<typename funct>
    callfunct_helper_pn<funct>
    callfunct(funct f, int port, bool &error_occurred, dlib::pipe<unsigned short> &port_pipe) {
    ...
    void dotest3() {
        ...
        bool error_occurred = false;
        {
            ...
            try {
                ...
                unsigned short port;
                ports.dequeue(port);
                hosts.push_back(network_address("127.0.0.1", port));
                dlog << LINFO << "PORT: " << port;
                ports.dequeue(port);
                hosts.push_back(network_address("127.0.0.1", port));
                dlog << LINFO << "PORT: " << port;
                ports.dequeue(port);
