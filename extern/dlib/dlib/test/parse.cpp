...
// ----------------------------------------------------------------------------------------
    ...
    void dotest1() {
        ...
        for (unsigned long i = 0; i < roots.size(); ++i) {
            ...
            DLIB_TEST(
                    parse_tree_to_string(parse_tree, words, roots[i]) == "[[The flight] [[includes [a meal]] AWORD]]");
            dlog << LINFO << parse_tree_to_string_tagged(parse_tree, words, roots[i]);
            DLIB_TEST(parse_tree_to_string_tagged(parse_tree, words, roots[i]) ==
                      "[5 [3 The flight] [4 [4 includes [3 a meal]] AWORD]]");
        }
