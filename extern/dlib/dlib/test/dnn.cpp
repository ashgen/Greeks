...
// ----------------------------------------------------------------------------------------
    ...
    void test_batch_normalize() {
        ...
        batch_normalize(DEFAULT_BATCH_NORM_EPS, dest, means, vars, 1, running_means, running_variances, src, gamma,
                        beta);
        ...
        cpu::batch_normalize_inference(DEFAULT_BATCH_NORM_EPS, dest3, src, gamma, beta, running_means,
                                       running_variances);
        ...
        auto grad_src = [&](long idx) {
            auto f = [&](float eps) {
                const float old = src.host()[idx];
                src.host()[idx] += eps;
                batch_normalize(DEFAULT_BATCH_NORM_EPS, dest, means, vars, 1, running_means, running_variances, src,
                                gamma, beta);
                float result = dot(gradient_input, dest);
                src.host()[idx] = old;
                return result;
        ...
        batch_normalize_gradient(DEFAULT_BATCH_NORM_EPS, gradient_input, means, vars, src, gamma, src_grad, gamma_grad,
                                 beta_grad);
    ...
    void test_batch_normalize_conv() {
        ...
        resizable_tensor running_means;
        resizable_tensor running_variances;
        batch_normalize_conv(DEFAULT_BATCH_NORM_EPS, dest, means, vars, 1, running_means, running_variances, src, gamma,
                             beta);
        const double scale =
                (src.num_samples() * src.nr() * src.nc()) / (src.num_samples() * src.nr() * src.nc() - 1.0);
        ...
        running_variances = mat(running_variances) / scale;
        batch_normalize_conv_inference(DEFAULT_BATCH_NORM_EPS, dest2, src, gamma, beta, running_means,
                                       running_variances);
        DLIB_TEST(max(abs(mat(dest2) - mat(dest))) < 1e-5);
        cpu::batch_normalize_conv_inference(DEFAULT_BATCH_NORM_EPS, dest3, src, gamma, beta, running_means,
                                            running_variances);
        ...
        auto grad_src = [&](long idx) {
            auto f = [&](float eps) {
                const float old = src.host()[idx];
                src.host()[idx] += eps;
                batch_normalize_conv(DEFAULT_BATCH_NORM_EPS, dest, means, vars, 1, running_means, running_variances,
                                     src, gamma, beta);
                float result = dot(gradient_input, dest);
                src.host()[idx] = old;
                return result;
        ...
        batch_normalize_conv_gradient(DEFAULT_BATCH_NORM_EPS, gradient_input, means, vars, src, gamma, src_grad,
...
// ----------------------------------------------------------------------------------------
    ...
    void test_basic_tensor_ops() {
...

        {
            ...
            A.set_size(1, 4);
            rnd.fill_uniform(A);
            matrix<float> AA = join_cols(mat(A), mat(A));
            AA = join_cols(mat(A), AA);
...

        {
            ...
            rnd.fill_uniform(src1);
            tt::affine_transform(src1, src1, 1, 2);
            src2 = src1;  // random in range [2, 3]
            dest1 = exp(mat(src1));
            tt::exp(dest2, src2);
...
// ----------------------------------------------------------------------------------------
    ...
    void test_visit_functions() {
...

        {
            ...
            size_t count = 0;
            visit_layer_parameter_gradients(pnet, [&](size_t i, tensor &) {
                hit[i] = true;
                ++count;
            });
            for (auto x: hit)
        ...
        }
        {
            std::vector<bool> hit(pnet.num_computational_layers, false);
            size_t count = 0;
            visit_layer_parameter_gradients(const_pnet, [&](size_t i, const tensor &) {
                hit[i] = true;
                ++count;
            });
            for (auto x: hit)
...

        {
            ...
            visit_layer_parameters(pnet, [&](size_t i, tensor &) {
                hit2[i] = true;
                ++count;
            });
            for (auto x: hit2)
        ...
        }
        {
            size_t count = 0;
            std::vector<bool> hit2(pnet.num_computational_layers, false);
            visit_layer_parameters(const_pnet, [&](size_t i, const tensor &) {
                hit2[i] = true;
                ++count;
            });
            for (auto x: hit2)
                ...
                for (long r = 0; r < dest.nr(); ++r) {
                    for (long c = 0; c < dest.nc(); ++c) {
                        ...
                        if (k < src1.k()) {
                            ...
                            float src_value =
                                    tensor_read_cpu(src3, i, k - src1.k() - src2.k() + 3, r, c) + old_dest_value;
                            DLIB_TEST(std::abs(src_value - dest_value) < 1e-6);
                        }
                    }
                }
...
// ----------------------------------------------------------------------------------------
    ...
    void test_simple_linear_regression() {
        ...
        DLIB_TEST_MSG(slope_error <= eps_slope,
                      "Expected slope = " << true_slope << " Estimated slope = " << slope << " Error limit = "
                                          << eps_slope);
        DLIB_TEST_MSG(intercept_error <= eps_intercept,
                      "Expected intercept = " << true_intercept << " Estimated intercept = " << intercept
                                              << " Error limit = " << eps_intercept);
...
// ----------------------------------------------------------------------------------------
    ...
    void test_multioutput_linear_regression() {
        ...
        for (int ii = 0; ii < num_samples; ++ii) {
            ...
            x[ii] = tmp;
            for (int jj = 0; jj < num_outputs; ++jj)
                ytmp(jj, 0) = (true_intercepts[jj] + true_slopes[jj] * static_cast<float>(val) +
        ...
        for (int i = 0; i < num_samples; ++i) {
            ...
            for (int r = 0; r < output_height; ++r)
                for (int c = 0; c < output_width; ++c)
                    switch (model) {
                        case 0:
                            tmp(r, c) = r / output_height;
                            break;
                        case 1:
                            tmp(r, c) = c / output_width;
                            break;
                        case 2:
                            tmp(r, c) = 1.0 - r / output_height;
                            break;
                        case 3:
                            tmp(r, c) = 1.0 - c / output_width;
                            break;
                        default:
                            DLIB_TEST_MSG(false, "Invalid model: " << model << " (should be between 0 and 3)");
                    }
...
// ----------------------------------------------------------------------------------------
    ...
    void test_loss_binary_log_per_pixel_with_noise_and_pixels_to_ignore() {
        ...
        { // Require a reasonably balanced truth histogram in order to make sure that a trivial classifier is not enough
            ...
            for (auto histogram_value: truth_histogram) {
                DLIB_TEST_MSG(histogram_value >= required_min_histogram_value,
                              "Histogram value = " << histogram_value << ", required = "
                                                   << required_min_histogram_value);
            }
        }
        ...
        DLIB_TEST_MSG(num_correct >= num_correct_required,
                      "Number of correctly classified elements = " << num_correct << ", required = "
            ...
            for (int jj = 0; jj < output_height; ++jj) {
                for (int kk = 0; kk < output_width; ++kk) {
                    ...
                    for (long k = 0; k < num_classes; ++k) {
                        const size_t index =
                                ((ii * output_tensor.k() + k) * output_tensor.nr() + jj) * output_tensor.nc() + kk;
...
// ----------------------------------------------------------------------------------------
    ...
    void test_loss_multiclass_per_pixel_with_noise_and_pixels_to_ignore() {
        ...
        { // Require a reasonably balanced truth histogram in order to make sure that a trivial classifier is not enough
            const int required_min_histogram_value = static_cast<int>(::std::ceil(
                    num_total_elements / num_classes * 0.375));
            for (auto histogram_value: truth_histogram) {
                DLIB_TEST_MSG(histogram_value >= required_min_histogram_value,
                              "Histogram value = " << histogram_value << ", required = "
                                                   << required_min_histogram_value);
...
// ----------------------------------------------------------------------------------------
    ...
    void test_tensor_resize_bilinear(long samps, long k, long nr, long nc, long onr, long onc) {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            DLIB_TEST_MSG(std::abs(numerical_grad - grad2.host()[idx] + 0.1) < 1e-2,
                          std::abs(numerical_grad - grad2.host()[idx] + 0.1) << "  numerical_grad: " << numerical_grad);
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            cpu::resize_bilinear_gradient(wgrad2, grad2.nc(), grad2.nr() * grad2.nc(), wgradient_input,
                                          gradient_input.nc(), gradient_input.nr() * gradient_input.nc());
            dlog << LINFO << "analytic grad: " << grad2.host()[idx] - 0.1;
            DLIB_TEST_MSG(std::abs(numerical_grad - grad2.host()[idx] + 0.1) < 1e-2,
                          std::abs(numerical_grad - grad2.host()[idx] + 0.1) << "  numerical_grad: " << numerical_grad);
