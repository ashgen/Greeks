...
namespace {
    ...
    void c_check_equal(
        ...
        for (long r = 0; r < a.nr(); ++r) {
            for (long c = 0; c < a.nc(); ++c) {
                ...
                DLIB_TEST_MSG(error < std::sqrt(std::numeric_limits<typename type::value_type>::epsilon()) * eps_mul,
                              "error: " << error <<
                                        "    eps: "
                                        << std::sqrt(std::numeric_limits<typename type::value_type>::epsilon()) *
                                           eps_mul);
            }
        }
    }
    ...
    void test_blas(long rows, long cols) {
        ...
        for (int k = 0; k < 6; ++k) {
            ...
            c_check_equal(tmp(trans(conj(trans(c_at)) * trans(conj(c_a)))),
                          trans(conj(trans(c_at)) * trans(conj(c_a))));
            dlog << LTRACE << "1.8";
            ...
            dlog << LTRACE << "1.11";
            check_equal(tmp(trans(subm(a, 1, 1, 2, 2) * subm(a, 1, 2, 2, 2))),
                        trans(subm(a, 1, 1, 2, 2) * subm(a, 1, 2, 2, 2)));
            dlog << LTRACE << "1.12";
            ...
            DLIB_TEST(abs((tmp + i) - ((c_rv4 * c_cv4)(0) + i)) <
                      std::sqrt(std::numeric_limits<type>::epsilon()) * eps_mul);
            DLIB_TEST(max(abs((rv4 * cv4 + 1.0) - ((rv4 * cv4)(0) + 1.0))) <
                      std::sqrt(std::numeric_limits<type>::epsilon()) * eps_mul);
