...
// ----------------------------------------------------------------------------------------
    ...
    void test_linearity_complex() {
        ...
        auto func = [&](long nr, long nc) {
            ...
            DLIB_TEST_MSG(diff < tol,
                          "diff " << diff << " not within tol " << tol << " where (nr,nc) = (" << nr << "," << nc << ")"
                                  << " type " << typelabel);
    ...
    void test_time_shift() {
        ...
        auto func = [&](size_t size, size_t time_shift) {
            ...
            for (long i = 0; i < f1.size(); i++)
                f2_expected(i) = f1(i) * std::polar<R>(R(1),
                                                       R(6.283185307179586476925286766559005768394338798 * time_shift *
                                                         i / size));

            const auto diff_real = max(squared(real(f2) - real(f2_expected)));
            const auto diff_imag = max(squared(imag(f2) - imag(f2_expected)));

            DLIB_TEST_MSG(diff_real < tol,
                          typelabel << " diff_real " << diff_real << " size " << size << " shift " << time_shift);
            DLIB_TEST_MSG(diff_imag < tol,
                          typelabel << " diff_real " << diff_imag << " size " << size << " shift " << time_shift);
        };
...
// ----------------------------------------------------------------------------------------
    ...
    void test_fftr_conjugacy_1D() {
        ...
        auto func = [&](long nc) {
            ...
            DLIB_TEST_MSG(diff < tol,
                          typelabel << " diff " << diff << " nr " << m1.nr() << " nc " << m1.nc() << " tol " << tol);
        };
...
// ----------------------------------------------------------------------------------------
    ...
    void test_vector_overload_outplace() {
        ...
        auto func = [&](long size) {
            ...
            DLIB_TEST_MSG(diff < tol,
                          "diff " << diff << " not within tol " << tol << " where (size) = (" << size << ")" << " type "
                                  << typelabel);
    ...
    void test_random_stftrs() {
        ...
        for (long nsamples = 512; nsamples < 1024; ++nsamples) {
            matrix<R> samples1 = matrix_cast<R>(randm(1, nsamples));
            matrix<R> samples2 = istftr(stftr(samples1, make_hann(), fftsize, wlen, wlen / 2), make_hann(), wlen,
                                        wlen / 2);
