...
namespace {
    ...
    !*/
    {
...

        {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                for (long r = 0; r < 14; ++r) {
                    for (long c = 0; c < 15; ++c) {
                        DLIB_TEST_MSG(img[r][c].red == r * 14 + c + 1,
                                      "got " << (int) img[r][c].red << "  but expected " << r * 14 + c + 1);
                        DLIB_TEST(img[r][c].green == r * 14 + c + 2);
                        DLIB_TEST(img[r][c].blue == r * 14 + c + 3);
                    }
                }
    ...
    void test_filtering2(int nr, int nc, dlib::rand &rnd) {
        ...
        DLIB_TEST_MSG(max(abs(subm(mat(imout), rect) - subm(out, rect))) < 1e-5,
        ...
        for (long nr = 0; nr < 3; ++nr) {
            for (long nc = 0; nc < 3; ++nc) {
                ...
                for (int k = 0; k < 5; ++k) {
                    ...
                    test_spatially_filter_image_separable_down_simple(downsample, img, out1, row_filter, col_filter, 3,
                                                                      true, true);
...
// ----------------------------------------------------------------------------------------
    ...
    void test_extract_image_chips() {
        ...
        for (int iter = 0; iter < 1000; ++iter) {
            ...
            DLIB_TEST_MSG(max(abs(chip - 255)) == 0,
                          "nr: " << nr << "  nc: " << nc << "  size: " << size << "  angle: " << angle
        ...
        // sure noting funny happens for float images.
        {
            ...
            chip_details details(centered_rect(center(get_rect(img)), nr, nc), size, angle);
            extract_image_chip(img, details, chip);
            DLIB_TEST_MSG(max(abs(chip - 1234.5)) == 0,
                          "nr: " << nr << "  nc: " << nc << "  size: " << size << "  angle: " << angle
                                 << " error: " << max(abs(chip - 255)));
        }
        ...
        for (int iter = 0; iter < 1000; ++iter) {
            ...
            chip_details details(centered_rect(center(get_rect(img)), wsize, wsize), chip_dims(ht.size(), ht.size()),
                                 angle);
