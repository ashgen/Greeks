...
namespace {
    ...
    void matrix_test2() {
...

        {
            ...
            DLIB_TEST(equal(round_zeros(inv_upper_triangular(trans(L)) * trans(L), 1e-10), identity_matrix<double>(3),
                            1e-10));
...

        {
            ...
            DLIB_TEST_MSG(
                    equal(round_zeros(inv_upper_triangular(trans(L)) * trans(L), 1e-10), identity_matrix<double>(6),
                          1e-10),
...

        {
            ...
            DLIB_TEST_MSG(dlib::equal(inv_lower_triangular(trans(m)), inv(trans(m)), 1e-10),
                          inv_lower_triangular(trans(m)) - inv(trans(m)));
...

        {
            ...
            for (long r = 0; r < a.nr(); ++r) {
                for (long c = 0; c < a.nc(); ++c) {
                    DLIB_TEST_MSG(m1(r, c) == std::max(a(r, c), b(r, c)),
                                  m1(r, c) << " : " << a(r, c) << " " << b(r, c));
...

        {
            ...
            DLIB_TEST((complex_matrix(ones_matrix<double>(3, 3), zeros_matrix<double>(3, 3)) ==
        ...
        {
            DLIB_TEST((uniform_matrix<double>(303, 303, 3) * identity_matrix<double>(303) ==
                       uniform_matrix<double, 303, 303>(3)));
            DLIB_TEST((uniform_matrix<double, 303, 303>(3) * identity_matrix<double, 303>() ==
                       uniform_matrix<double, 303, 303>(3)));
        }
        ...
        }
        {
            ...
            for (long i = 0; i < x.nr(); ++i) DLIB_CASSERT(y(i) == std::exp(x(i) - max_val) * scale);
        }
    }
