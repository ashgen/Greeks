...
// ----------------------------------------------------------------------------------------
    ...
    namespace impl {
        ...
        template<typename T>
        using has_set_bias_learning_rate_multiplier = decltype(std::declval<T>().set_bias_learning_rate_multiplier(
                double{}));
        ...
        template<typename T>
        using has_set_bias_weight_decay_multiplier = decltype(std::declval<T>().set_bias_weight_decay_multiplier(
                double{}));
...
// ----------------------------------------------------------------------------------------
    ...
    // add_skip_layer).
    template<typename T>
    struct is_nonloss_layer_type : std::false_type {
    };
    // Tell us if T is an instance of add_loss_layer.
    template<typename T>
    struct is_loss_layer_type : std::false_type {
    };
    // Tell us if T is an instance of add_layer
    template<typename T>
    struct is_add_layer : std::false_type {
    };
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        add_layer &operator=(const add_layer &item) {
            add_layer(item).swap(*this);
            return *this;
        }

        add_layer(add_layer &&item) : add_layer() { swap(item); }

        add_layer &operator=(add_layer &&item) {
            swap(item);
            return *this;
        }
...
// ----------------------------------------------------------------------------------------
    ...
    private:
        ...
        tensor &private_get_output() const { return subnetwork.private_get_output(); }

        tensor &private_get_gradient_input() { return subnetwork.private_get_gradient_input(); }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        typedef typename subnet_type::input_layer_type input_layer_type;
        typedef int layer_details_type; // not really used anywhere, but required by subnet_wrapper.
        const static size_t comp_layers_in_each_group = (REPEATED_LAYER<SUBNET>::num_computational_layers -
                                                         SUBNET::num_computational_layers);
        ...
        void back_propagate_error(
            ...
            if (details.size() > 1) {
                ...
                for (size_t i = 1; i < details.size(); ++i) {
                    if (i + 1 < details.size())
                        details[i].back_propagate_error(details[i + 1].get_output(),
                                                        details[i - 1].get_final_data_gradient(), zero_grads);
                    else
                        details[i].back_propagate_error(subnetwork.get_output(),
                                                        details[i - 1].get_final_data_gradient(), zero_grads);
                }
            } else {
...
// is just an input layer object.
    ...
    public:
        ...
        add_tag_layer &operator=(add_tag_layer &&item) {
            swap(item);
            return *this;
        }
    ...
    private:
        ...
        tensor &private_get_output() const { return const_cast<tensor &>(get_output()); }
...
// ----------------------------------------------------------------------------------------
    ...
    private:
        ...
        template<typename LOSS_DETAILS, typename SUBNET> friend
        class add_loss_layer;

        template<typename net_type, typename solver_type> friend
        class dnn_trainer;
    };
...
// ----------------------------------------------------------------------------------------
    ...
    class add_loss_layer {
        ...
        template<typename T>
        struct get_loss_layer_training_label_type<T, typename std::enable_if<
                sizeof(typename T::training_label_type) != 0>::type> {
        ...
        template<typename T>
        struct get_loss_layer_output_label_type<T, typename std::enable_if<
                sizeof(typename T::output_label_type) != 0>::type> {
    ...
    public:
        ...
        add_loss_layer &operator=(add_loss_layer &&item) {
            swap(item);
            return *this;
        }
...
// ----------------------------------------------------------------------------------------
    ...
    namespace impl {
        ...
        >
        struct layer_helper<i, repeat<N, L, S>, typename std::enable_if<(i != 0 && i >=
                                                                                   repeat<N, L, S>::layers_in_repeated_group)>::type> {
        ...
        struct layer_helper<i, repeat<N, L, S>, typename std::enable_if<(i != 0 && i <
            ...
            static type &layer(repeat<N, L, S> &n) {
                return layer_helper<i % layers_in_each_group, next_type>::layer(
                        n.get_repeated_layer(i / layers_in_each_group));
            }
        };
        ...
        template<
                ...
                size_t N, template<typename> class L, typename S
        >
        struct layer_helper<i, const repeat<N, L, S>, typename std::enable_if<(i != 0 && i >=
                                                                                         repeat<N, L, S>::layers_in_repeated_group)>::type> {
            const static size_t layers_in_repeated_group = repeat<N, L, S>::layers_in_repeated_group;
...
// ----------------------------------------------------------------------------------------
    ...
    private:
        ...
        tensor &private_get_output() const { return layer<TAG_TYPE>(subnetwork).private_get_output(); }

        tensor &private_get_gradient_input() { return layer<TAG_TYPE>(subnetwork).private_get_gradient_input(); }
...
// ----------------------------------------------------------------------------------------
    ...
    namespace timpl {
        ...
        public:
            ...
            const test_layer_subnet &subnet() const {
                init_sub();
                return *subnetwork;
            }
            ...
            if (impl::is_inplace_layer(l, subnetwork)) {
                ...
                if (params_grad.size() != 0) {
                    ...
                    if (backward_param_error > 0.00001) {
                        ...
                        sout << "changes when invoked in-place vs. out-of-place. The error was: "
                             << backward_param_error << endl;
                        return layer_test_results(sout.str());
                    }
                }
                ...
                if (backward_data_error > 0.00001) {
                    ...
                    sout
                            << "This layer is supposed to support in-place computations but the output of backward_inplace()\n";
                    sout << "changes when invoked in-place vs. out-of-place. The error was: " << backward_data_error
                         << endl;
                    return layer_test_results(sout.str());
                }
            }
    ...
    namespace impl {
        ...
        public:
            ...
            void operator()(layer &l) {
                ...
                DLIB_CASSERT(visitor_called,
                             "A visitor function with an incorrect signature was given to visit_layer_parameters()");
    ...
    namespace impl {
        ...
        public:
            ...
            void do_visit(layer &l) {
                ...
                DLIB_CASSERT(visitor_called,
                             "A visitor function with an incorrect signature was given to visit_layer_parameter_gradients()");
                ++computational_layer_idx;
            }
