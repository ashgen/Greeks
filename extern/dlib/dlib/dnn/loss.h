        ...
        friend void deserialize(loss_multiclass_log_weighted_ &, std::istream &in) {
            ...
            if (version != "loss_multiclass_log_weighted_")
                throw serialization_error(
                        "Unexpected version found while deserializing dlib::loss_multiclass_log_weighted_.");
        }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        void to_label(
            ...
            DLIB_CASSERT(number_of_labels() != 0,
                         "You must give the loss_multimulticlass_log_'s constructor label data before you can use it!");
            DLIB_CASSERT(output_tensor.k() == (long) number_of_labels(),
                         "The output tensor must have " << number_of_labels() << " channels.");
        ...
        friend void deserialize(loss_multimulticlass_log_ &item, std::istream &in) {
            ...
            if (version != "loss_multimulticlass_log_")
                throw serialization_error(
                        "Unexpected version found while deserializing dlib::loss_multimulticlass_log_.");
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        struct detector_window_details {
            ...
            detector_window_details(unsigned long w, unsigned long h, const std::string &l) : width(w), height(h),
                                                                                              label(l) {}
            ...
            friend inline void deserialize(detector_window_details &item, std::istream &in) {
                ...
                if (version != 1 && version != 2)
                    throw serialization_error(
                            "Unexpected version found while deserializing dlib::mmod_options::detector_window_details");
        ...
        mmod_options(
            ...
            for (auto &label: get_labels(boxes)) {
                for (auto ratio: find_covering_aspect_ratios(boxes, test_box_overlap(min_detector_window_overlap_iou),
                    ...
                    detector_window_details p((unsigned long) std::round(detector_width),
                                              (unsigned long) std::round(detector_height), label);
                    detector_windows.push_back(p);
                }
            }

            DLIB_CASSERT(detector_windows.size() != 0,
                         "You can't call mmod_options's constructor with a set of boxes that is empty (or only contains ignored boxes).");
        ...
        mmod_options(
            ...
            // Figure out what detector windows we will need.
            for (auto &label: get_labels(boxes)) {
                for (auto rectangle: find_covering_rectangles(boxes, test_box_overlap(min_detector_window_overlap_iou),
                                                              label)) {
    ...
    inline std::ostream &
    operator<<(std::ostream &out, const std::vector<mmod_options::detector_window_details> &detector_windows) {
...
// ----------------------------------------------------------------------------------------
    ...
    class loss_mmod_ {
        struct intermediate_detection {
            ...
            intermediate_detection(
                ...
                tensor_channel(channel), rect_bbr(rect_) {}
            ...
            bool operator<(const intermediate_detection &item) const {
    ...
    public:
        ...
        double compute_loss_value_and_gradient(
            ...
            const double scale = 1.0 / (output_tensor.nr() * output_tensor.nc() * output_tensor.num_samples() *
                                        options.detector_windows.size());
            double loss = 0;
            ...
            for (long i = 0; i < output_tensor.num_samples(); ++i) {
                tensor_to_dets(input_tensor, output_tensor, i, dets,
                               -options.loss_per_false_alarm + det_thresh_speed_adjust, sub);
                ...
                if (dets.size() > max_num_initial_dets) {
                    det_thresh_speed_adjust = std::max(det_thresh_speed_adjust,
                                                       dets[max_num_initial_dets].detection_confidence +
                                                       options.loss_per_false_alarm);
                }
                    ...
                    if (!x.ignore) {
                        ...
                        if (i != idx_to_truth_rect.end()) {
                            if (!options.be_quiet) {
                                ...
                                std::cout << "Warning, ignoring object.  We encountered a truth rectangle located at "
                                          << x.rect;
                                std::cout
                        ...
                        if (options.overlaps_nms(best_matching_truth_box, (*truth)[i])) {
                            ...
                            if (idx != -1) {
                                ...
                                if (!options.be_quiet) {
                                    ...
                                    std::cout << "because it is overlapped by another truth rectangle located at "
                                              << best_matching_truth_box
                                              << " (IoU:"
                                              << box_intersection_over_union(best_matching_truth_box, (*truth)[i])
                                              << ", Percent covered:"
                                              << box_percent_covered(best_matching_truth_box, (*truth)[i]) << ")."
                                              << std::endl;
                                }
                            }
                        }
        ...
        friend std::ostream &operator<<(std::ostream &out, const loss_mmod_ &item) {
            ...
            out << ", overlaps_ignore:(" << opts.overlaps_ignore.get_iou_thresh() << ","
                << opts.overlaps_ignore.get_percent_covered_thresh() << ")";
    ...
    private:
        ...
        void tensor_to_dets(
            ...
            const float *out_data =
                    output_tensor.host() + output_tensor.k() * output_tensor.nr() * output_tensor.nc() * i;
                ...
                for (long r = 0; r < output_tensor.nr(); ++r) {
                    for (long c = 0; c < output_tensor.nc(); ++c) {
                        ...
                        if (score > adjust_threshold) {
                            ...
                            drectangle rect = centered_drect(p, options.detector_windows[k].width,
                                                             options.detector_windows[k].height);
                            rect = input_layer(net).tensor_space_to_image_space(input_tensor, rect);

                            dets_accum.push_back(intermediate_detection(rect, score, (k * output_tensor.nr() + r) *
                                                                                     output_tensor.nc() + c, k));

                            if (options.use_bounding_box_regression) {
                                const auto offset = options.detector_windows.size() + k * 4;
                                dets_accum.back().tensor_offset_dx =
                                        ((offset + 0) * output_tensor.nr() + r) * output_tensor.nc() + c;
                                dets_accum.back().tensor_offset_dy =
                                        ((offset + 1) * output_tensor.nr() + r) * output_tensor.nc() + c;
                                dets_accum.back().tensor_offset_dw =
                                        ((offset + 2) * output_tensor.nr() + r) * output_tensor.nc() + c;
                                dets_accum.back().tensor_offset_dh =
                                        ((offset + 3) * output_tensor.nr() + r) * output_tensor.nc() + c;
        ...
        size_t find_best_detection_window(
            ...
            for (size_t i = 0; i < options.detector_windows.size(); ++i) {
                ...
                if (options.assume_image_pyramid == use_image_pyramid::yes) {
                    det_window = centered_rect(point(0, 0), options.detector_windows[i].width,
                                               options.detector_windows[i].height);
        ...
        bool image_rect_to_feat_coord(
            ...
            if (options.assume_image_pyramid == use_image_pyramid::yes) {
                ...
                scale = std::max(options.detector_windows[det_idx].width / (double) rect.width(),
                                 options.detector_windows[det_idx].height / (double) rect.height());
            } else {
                // We don't want invariance to scale.
                scale = 1.0;
            }

            const rectangle mapped_rect = input_layer(net).image_space_to_tensor_space(input_tensor,
                                                                                       std::min(1.0, scale), rect);

            // compute the detection window that we would use at this position.
            tensor_p = center(mapped_rect);
            rectangle det_window = centered_rect(tensor_p, options.detector_windows[det_idx].width,
                                                 options.detector_windows[det_idx].height);
            ...
            if (box_intersection_over_union(rect, det_window) <= options.truth_match_iou_threshold) {
                std::cout << "Warning, ignoring object.  We encountered a truth rectangle with a width and height of "
                          << rect.width() << " and " << rect.height() << ".  ";
                std::cout << "The image pyramid and sliding windows can't output a rectangle of this shape.  ";
                const double detector_area =
                        options.detector_windows[det_idx].width * options.detector_windows[det_idx].height;
                if (mapped_rect.area() / detector_area <= options.truth_match_iou_threshold) {
                    std::cout
                            << "This is because the rectangle is smaller than the best matching detection window, which has a width ";
                    std::cout << "and height of " << options.detector_windows[det_idx].width << " and "
                              << options.detector_windows[det_idx].height << "." << std::endl;
                } else {
                    std::cout
                            << "This is either because (1) the final layer's features have too large of a stride across the image, limiting the possible locations the sliding window can search ";
                    std::cout
                            << "or (2) because the rectangle's aspect ratio is too different from the best matching detection window, ";
                    std::cout << "which has a width and height of " << options.detector_windows[det_idx].width
                              << " and " << options.detector_windows[det_idx].height << "." << std::endl;
                }
                return true;
            }
            ...
            if (!get_rect(output_tensor).contains(tensor_p)) {
                std::cout << "Warning, ignoring object.  We encountered a truth rectangle located at " << rect
    ...
    public:
        ...
        friend void deserialize(loss_metric_ &item, std::istream &in) {
            ...
            if (version == "loss_metric_") {
                ...
                throw serialization_error(
                        "Unexpected version found while deserializing dlib::loss_metric_.  Instead found " + version);
            }
        }
    ...
    public:
        ...
        friend void deserialize(loss_epsilon_insensitive_ &item, std::istream &in) {
            ...
            if (version != "loss_epsilon_insensitive_")
                throw serialization_error(
                        "Unexpected version found while deserializing dlib::loss_epsilon_insensitive_.");
    ...
    public:
        ...
        friend void deserialize(loss_binary_log_per_pixel_ &, std::istream &in) {
            ...
            if (version != "loss_binary_log_per_pixel_")
                throw serialization_error(
                        "Unexpected version found while deserializing dlib::loss_binary_log_per_pixel_.");
        }
    ...
    public:
        ...
        friend void deserialize(loss_multiclass_log_per_pixel_ &, std::istream &in) {
            ...
            if (version != "loss_multiclass_log_per_pixel_")
                throw serialization_error(
                        "Unexpected version found while deserializing dlib::loss_multiclass_log_per_pixel_.");
        }
    ...
    public:
        ...
        friend void deserialize(loss_mean_squared_per_pixel_ &, std::istream &in) {
            ...
            if (version != "loss_mean_squared_per_pixel_")
                throw serialization_error(
                        "Unexpected version found while deserializing dlib::loss_mean_squared_per_pixel_.");
        }
    ...
    public:
                ...
                for (long k = 0; k < output_tensor.k(); ++k) {
                    DLIB_CASSERT((*truth_matrix_ptr)[k].nr() == output_tensor.nr() &&
                                 ...
                                 "truth size = " << (*truth_matrix_ptr)[k].nr() << " x " << (*truth_matrix_ptr)[k].nc()
                                                 << ", "
                                                    "output size = " << output_tensor.nr() << " x "
                                                 << output_tensor.nc());
                }
            }
            double loss;
        ...
        friend void deserialize(loss_mean_squared_per_channel_and_pixel_ &, std::istream &in) {
            ...
            if (version != "loss_mean_squared_per_channel_and_pixel_")
                throw serialization_error(
                        "Unexpected version found while deserializing dlib::loss_mean_squared_per_channel_and_pixel_.");
        }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        double compute_loss_value_and_gradient(
            ...
            const float *out_data = output_tensor.host();
            for (long i = 0; i < output_tensor.num_samples(); ++i) {
                DLIB_CASSERT(truth->size() == network_output_dims,
...
// ----------------------------------------------------------------------------------------
    ...
    inline std::ostream &
    operator<<(std::ostream &out, const std::map<int, std::vector<yolo_options::anchor_box_details>> &anchors) {
    ...
    public:
        ...
        void to_label(
            ...
            for (long i = 0; i < input_tensor.num_samples(); ++i) {
                dets_accum.clear();
                impl::yolo_helper_impl<TAG_TYPES...>::tensor_to_dets(input_tensor, sub, i, options, adjust_threshold,
                                                                     dets_accum);
    ...
    template<template<typename> class TAG_1,
            template<typename> class TAG_2,
            template<typename> class TAG_3, typename SUBNET>
    ...
    public:
        ...
        friend void deserialize(loss_barlow_twins_ &item, std::istream &in) {
            ...
            if (version == "loss_barlow_twins_") {
                ...
                throw serialization_error(
                        "Unexpected version found while deserializing dlib::loss_barlow_twins_.  Instead found " +
                        version);
            }
        }
