    ...
    private:
            ...
            while (job_pipe.dequeue(next_job)) {
                if (next_job.test_only) {
                    ...
                    for (size_t i = 0; i < devices.size(); ++i)
                        tp[i]->add_task_by_value([&, i](double &loss) {
                            loss = compute_parameter_gradients(i, next_job, pick_which_run_update);
                        }, losses[i]);
                    ...
                    if (learning_rate_shrink != 1) {
                        ...
                        if (test_steps_without_progress >= test_iter_without_progress_thresh) {
                            test_steps_without_progress = count_steps_without_decrease_robust(
        ...
        friend void deserialize(dnn_trainer &item, std::istream &in) {
            ...
            if (num_layers != dnn_trainer::num_layers) {
                ...
                sout << "Error deserializing dlib::dnn_trainer.  The saved sync file is for a network with "
                     << std::endl;
                sout << "a different number of layers.  We expected the number of layers to be "
                     << dnn_trainer::num_layers << " but" << std::endl;
            ...
            deserialize(dtemp, in);
            item.learning_rate = dtemp;
            deserialize(item.min_learning_rate, in);
            deserialize(ltemp, in);
            item.iter_without_progress_thresh = ltemp;
            deserialize(ltemp, in);
            item.steps_without_progress = ltemp;
            deserialize(dtemp, in);
            item.learning_rate_shrink = dtemp;
            ...
            deserialize(ltemp, in);
            item.test_iter_without_progress_thresh = ltemp;
            deserialize(ltemp, in);
            item.test_steps_without_progress = ltemp;
        ...
        void drop_some_previous_loss_values() {
            for (unsigned long cnt = 0; cnt < previous_loss_values_dump_amount + iter_without_progress_thresh / 10 &&
                                        previous_loss_values.size() > 0; ++cnt)
                previous_loss_values.pop_front();
        }
        ...
        void drop_some_test_previous_loss_values() {
            for (unsigned long cnt = 0;
                 cnt < test_previous_loss_values_dump_amount + test_iter_without_progress_thresh / 10 &&
                 test_previous_loss_values.size() > 0; ++cnt)
                test_previous_loss_values.pop_front();
        }
        ...
        void sync_to_disk(
            ...
            if (std::chrono::system_clock::now() - last_sync_time > time_between_syncs ||
                ...
                if (loss_increased_since_last_disk_sync()) {
                    ...
                    if (verbose)
                        std::cout << "Loss has been increasing, reloading saved state from " << newest_syncfile()
                                  << std::endl;
        ...
        bool loss_increased_since_last_disk_sync() {
            ...
            if (std::max(prob1, prob2) > prob_loss_increasing_thresh) {
                ...
                prob_loss_increasing_thresh = std::max(prob_loss_increasing_thresh,
        ...
        struct device_data {
            ...
            device_data(
                    ...
                    const solver_type &solver_,
                    clone_net
            ) : device_id(device_id_), net_copy(std::make_shared<net_type>(net_)), net(*net_copy),
                solvers(num_computational_layers, solver_) {}
        ...
        void print_progress() {
            if (lr_schedule.size() == 0) {
                if (test_previous_loss_values.size() == 0)
                    ...
                    std::cout << "steps without apparent progress: train=" << steps_without_progress << ", test="
                              << test_steps_without_progress;
            } else {
                std::ostringstream sout;
                sout << "percent complete: " << std::fixed << std::setprecision(2)
                     << 100.0 * lr_schedule_pos / (double) lr_schedule.size() << "%";
                std::cout << sout.str();
            }
            std::cout << std::endl;
        }
...
// ----------------------------------------------------------------------------------------
    ...
    std::ostream &operator<<(
        ...
        if (sched.size() != 0) {
            ...
            out << "  learning rate shrink factor:                " << trainer.get_learning_rate_shrink_factor()
                << endl;
            out << "  min learning rate:                          " << trainer.get_min_learning_rate() << endl;
            out << "  iterations without progress threshold:      "
                << trainer.get_iterations_without_progress_threshold() << endl;
            out << "  test iterations without progress threshold: "
                << trainer.get_test_iterations_without_progress_threshold() << endl;
        }
        return out;
    }
