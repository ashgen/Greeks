...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        static_assert(_nr == 0 || (0 <= _padding_y && _padding_y < _nr),
                      "The padding must be smaller than the filter size.");
        static_assert(_nc == 0 || (0 <= _padding_x && _padding_x < _nc),
                      "The padding must be smaller than the filter size.");
        ...
        friend void deserialize(con_ &item, std::istream &in) {
            ...
            if (version == "con_4" || version == "con_5") {
                ...
                if (item.padding_y_ != _padding_y)
                    throw serialization_error("Wrong padding_y found while deserializing dlib::con_");
                if (item.padding_x_ != _padding_x)
                    throw serialization_error("Wrong padding_x found while deserializing dlib::con_");
                ...
                if (stride_y != _stride_y)
                    throw serialization_error("Wrong stride_y found while deserializing dlib::con_");
                if (stride_x != _stride_x)
                    throw serialization_error("Wrong stride_x found while deserializing dlib::con_");
                if (version == "con_5") {
        ...
        friend void deserialize(upsample_ & /*item*/, std::istream &in) {
            ...
            if (version != "upsample_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::upsample_.");
    ...
    public:
        ...
        friend void deserialize(resize_to_ &item, std::istream &in) {
            ...
            if (version != "resize_to_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::resize_to_.");
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        template<typename SUBNET>
        void
        backward(const tensor &computed_output, const tensor &gradient_input, SUBNET &sub, tensor & /*params_grad*/) {
            mp.setup_max_pooling(_nr != 0 ? _nr : sub.get_output().nr(),
                                 _nc != 0 ? _nc : sub.get_output().nc(),
                                 _stride_y, _stride_x, padding_y_, padding_x_);
        ...
        friend void deserialize(max_pool_ &item, std::istream &in) {
            ...
            if (version == "max_pool_2") {
                ...
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::max_pool_.");
            }

            if (item.padding_y_ != _padding_y)
                throw serialization_error("Wrong padding_y found while deserializing dlib::max_pool_");
            if (item.padding_x_ != _padding_x)
                throw serialization_error("Wrong padding_x found while deserializing dlib::max_pool_");
            ...
            if (_stride_y != stride_y)
                throw serialization_error("Wrong stride_y found while deserializing dlib::max_pool_");
            if (_stride_x != stride_x)
                throw serialization_error("Wrong stride_x found while deserializing dlib::max_pool_");
        }
    ...
    public:
        ...
        friend void deserialize(avg_pool_ &item, std::istream &in) {
            ...
            if (version == "avg_pool_2") {
                ...
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::avg_pool_.");
            }

            if (item.padding_y_ != _padding_y)
                throw serialization_error("Wrong padding_y found while deserializing dlib::avg_pool_");
            if (item.padding_x_ != _padding_x)
                throw serialization_error("Wrong padding_x found while deserializing dlib::avg_pool_");
            ...
            if (_stride_y != stride_y)
                throw serialization_error("Wrong stride_y found while deserializing dlib::avg_pool_");
            if (_stride_x != stride_x)
                throw serialization_error("Wrong stride_x found while deserializing dlib::avg_pool_");
        }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        void backward(const tensor &gradient_input, SUBNET &sub, tensor &params_grad) {
            ...
            tt::layer_normalize_gradient(eps, gradient_input, means, invstds, sub.get_output(), g,
                                         sub.get_gradient_input(), g_grad, b_grad);
        }
        ...
        friend void deserialize(layer_norm_ &item, std::istream &in) {
            ...
            if (version != "layer_norm_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::layer_norm_.");
            deserialize(item.params, in);
    ...
    public:
        ...
        void forward(const SUBNET &sub, resizable_tensor &output) {
            ...
            if (sub.get_output().num_samples() > 1) {
                ...
                if (mode == FC_MODE)
                    tt::batch_normalize(eps, output, means, invstds, decay, running_means, running_variances,
                                        sub.get_output(), g, b);
                else
                    tt::batch_normalize_conv(eps, output, means, invstds, decay, running_means, running_variances,
                                             sub.get_output(), g, b);
            } else // we are running in testing mode so we just linearly scale the input tensor.
            {
                if (mode == FC_MODE)
                    tt::batch_normalize_inference(eps, output, sub.get_output(), g, b, running_means,
                                                  running_variances);
                else
                    tt::batch_normalize_conv_inference(eps, output, sub.get_output(), g, b, running_means,
                                                       running_variances);
            }
        }
        ...
        void backward(const tensor &gradient_input, SUBNET &sub, tensor &params_grad) {
            ...
            auto b_grad = beta(params_grad, gamma.size());
            if (mode == FC_MODE)
                tt::batch_normalize_gradient(eps, gradient_input, means, invstds, sub.get_output(), g,
                                             sub.get_gradient_input(), g_grad, b_grad);
            else
                tt::batch_normalize_conv_gradient(eps, gradient_input, means, invstds, sub.get_output(), g,
                                                  sub.get_gradient_input(), g_grad, b_grad);
        }
        ...
        friend void deserialize(bn_ &item, std::istream &in) {
            ...
            if (mode == CONV_MODE) {
                if (version != "bn_con2")
                    throw serialization_error(
                            "Unexpected version '" + version + "' found while deserializing dlib::bn_.");
    ...
    public:
        ...
        friend void deserialize(fc_ &item, std::istream &in) {
            ...
            if (version == "fc_2" || version == "fc_3") {
                ...
                if (bias_mode != (fc_bias_mode) bmode)
                    throw serialization_error("Wrong fc_bias_mode found while deserializing dlib::fc_");
                deserialize(item.learning_rate_multiplier, in);
                deserialize(item.weight_decay_multiplier, in);
                deserialize(item.bias_learning_rate_multiplier, in);
                deserialize(item.bias_weight_decay_multiplier, in);
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void deserialize(dropout_ &item, std::istream &in) {
            ...
            if (version != "dropout_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::dropout_.");
    ...
    public:
        ...
        friend void deserialize(multiply_ &item, std::istream &in) {
            ...
            if (version != "multiply_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::multiply_.");
    ...
    public:
        ...
        friend void deserialize(affine_ &item, std::istream &in) {
            ...
            if (version != "affine_" && version != "affine_2")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::affine_.");
            deserialize(item.params, in);
            deserialize(item.gamma, in);
            deserialize(item.beta, in);
            int mode;
    ...
    public:
        ...
        friend void deserialize(add_prev_ & /*item*/, std::istream &in) {
            ...
            if (version != "add_prev_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::add_prev_.");
        }
    ...
    public:
        ...
        friend void deserialize(mult_prev_ & /*item*/, std::istream &in) {
            ...
            if (version != "mult_prev_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::mult_prev_.");
        }
    ...
    public:
        ...
        friend void deserialize(resize_prev_to_tagged_ & /*item*/, std::istream &in) {
            ...
            if (version != "resize_prev_to_tagged_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::resize_prev_to_tagged_.");
        }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void deserialize(scale_ &item, std::istream &in) {
            ...
            if (version != "scale_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::scale_.");
    ...
    public:
        ...
        friend void deserialize(scale_prev_ &item, std::istream &in) {
            ...
            if (version != "scale_prev_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::scale_prev_.");
            deserialize(item.reshape_scales, in);
            deserialize(item.reshape_src, in);
        }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void deserialize(relu_ & /*item*/, std::istream &in) {
            ...
            if (version != "relu_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::relu_.");
        }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void deserialize(prelu_ &item, std::istream &in) {
            ...
            if (version != "prelu_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::prelu_.");
            deserialize(item.params, in);
    ...
    public:
        ...
        friend void deserialize(leaky_relu_ &item, std::istream &in) {
            ...
            if (version != "leaky_relu_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::leaky_relu_.");
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void deserialize(mish_ & /*item*/, std::istream &in) {
            ...
            if (version != "mish_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::mish_.");
        }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void deserialize(htan_ & /*item*/, std::istream &in) {
            ...
            if (version != "htan_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::htan_.");
        }
    ...
    public:
        ...
        friend void deserialize(clipped_relu_ &item, std::istream &in) {
            ...
            if (version != "clipped_relu_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::clipped_relu_.");
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void deserialize(gelu_ & /*item*/, std::istream &in) {
            ...
            if (version != "gelu_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::gelu_.");
        }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void deserialize(smelu_ &item, std::istream &in) {
            ...
            if (version != "smelu_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::smelu_.");
            deserialize(item.beta, in);
        }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void deserialize(silu_ & /*item*/, std::istream &in) {
            ...
            if (version != "silu_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::silu_.");
        }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void deserialize(softmax_ & /*item*/, std::istream &in) {
            ...
            if (version != "softmax_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::softmax_.");
        }
    ...
    public:
        ...
        friend void deserialize(softmax_all_ & /*item*/, std::istream &in) {
            ...
            if (version != "softmax_all_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::softmax_all_.");
        }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void deserialize(concat_ & /*item*/, std::istream &in) {
            ...
            if (version != "concat_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::concat_.");
            size_t count_tags;
    ...
    public:
        ...
        friend void deserialize(l2normalize_ &item, std::istream &in) {
            ...
            if (version != "l2normalize_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::l2normalize_.");
            deserialize(item.eps, in);
        }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        void setup(const SUBNET &sub) {
            ...
            ain = alias_tensor(sub.get_output().num_samples(),
                               sub.get_output().size() / sub.get_output().num_samples());
        }
        ...
        friend void deserialize(extract_ & /*item*/, std::istream &in) {
            ...
            if (version != "extract_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::extract_.");
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void deserialize(reorg_ & /*item*/, std::istream &in) {
            ...
            if (version != "reorg_")
                throw serialization_error(
                        "Unexpected version '" + version + "' found while deserializing dlib::reorg_.");
            long long rs;
            long long cs;
            deserialize(rs, in);
            deserialize(cs, in);
