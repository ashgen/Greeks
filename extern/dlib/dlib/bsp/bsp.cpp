    ...
    namespace impl2 {
        ...
        void read_thread(
            ...
            try {
                ...
                sout << "An exception was thrown while attempting to receive a message from processing node "
                     << sender_id << ".\n";
                sout << "  Sending processing node address:   " << con->con->get_foreign_ip() << ":"
                     << con->con->get_foreign_port() << std::endl;
                sout << "  Receiving processing node address: " << con->con->get_local_ip() << ":"
                     << con->con->get_local_port() << std::endl;
...
// ----------------------------------------------------------------------------------------
    ...
    close_all_connections_gracefully(
        ...
        while (num_terminated_nodes < _cons.size()) {
            if (node_id() == 0 && num_waiting_nodes + num_terminated_nodes == _cons.size() &&
        ...
        if (node_id() == 0) {
            ...
            if (outstanding_messages != 0) {
                ...
                sout << "There are at least " << outstanding_messages
...
// ----------------------------------------------------------------------------------------
    ...
    receive_data(
        ...
        while (true) {
            ...
            if (node_id() == 0 && outstanding_messages == 0 &&
                num_terminated_nodes + num_waiting_nodes == _cons.size()) {
                num_waiting_nodes = 0;
                broadcast_byte(impl2::SEE_ALL_IN_WAITING_STATE);
    ...
    notify_control_node(
        ...
        if (node_id() == 0) {
            ...
            switch (val) {
                ...
                default: DLIB_CASSERT(false, "This should never happen");
            }
        } else {
