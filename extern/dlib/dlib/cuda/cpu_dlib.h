...
namespace dlib {
    namespace cpu {
        ...
        inline void resize_bilinear_gradient(
            ...
            resize_bilinear_gradient(grad, grad.nc(), grad.nr() * grad.nc(), gradient_input, gradient_input.nc(),
        ...
        public:
            ...
            void operator()(
                ...
                // The loss we output is the average loss over the mini-batch, and also over each element of the matrix output.
                const double scale = 1.0 / (output_tensor.num_samples() * output_tensor.k() * output_tensor.nr() *
                                            output_tensor.nc());
                loss = 0;
                float *const g = grad.host();
