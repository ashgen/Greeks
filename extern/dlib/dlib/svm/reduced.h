...
// ----------------------------------------------------------------------------------------
    ...
    private:
        ...
        const decision_function<kernel_type> do_train(
            ...
            alpha = lisf.get_inv_kernel_marix() *
                    (kernel_matrix(kern, lisf, dec_funct.basis_vectors) * dec_funct.alpha);
...
// ----------------------------------------------------------------------------------------
    ...
    distance_function<K> approximate_distance_function(
        ...
        beta = pinv(kernel_matrix(kern, out_vectors)) *
               (kernel_matrix(kern, out_vectors, target.get_basis_vectors()) * target.get_alpha());
