...
// ----------------------------------------------------------------------------------------
    ...
    namespace impl {
        ...
        inline unsigned long number_of_truth_hits(
            ...
            return number_of_truth_hits(truth_boxes, ignore, boxes, overlap_tester, all_dets, missing_detections,
                                        overlap_tester);
        }
...
// ----------------------------------------------------------------------------------------
    ...
    const matrix<double, 1, 3> test_object_detection_function(
        ...
        for (unsigned long i = 0; i < images.size(); ++i) {
            ...
            correct_hits += impl::number_of_truth_hits(truth_dets[i], ignore[i], hits, overlap_tester, all_dets,
                                                       missing_detections);
...
// ----------------------------------------------------------------------------------------
    ...
    const matrix<double, 1, 3> cross_validate_object_detection_trainer(
        ...
        for (long iter = 0; iter < folds; ++iter) {
            ...
            typename trainer_type::trained_function_type detector = trainer.train(array_subset, training_rects,
                                                                                  training_ignores, overlap_tester);
            for (unsigned long i = 0; i < test_idx_set.size(); ++i) {
                std::vector<std::pair<double, rectangle> > hits;
                detector(images[test_idx_set[i]], hits, adjust_threshold);

                correct_hits += impl::number_of_truth_hits(truth_dets[test_idx_set[i]], ignore[i], hits, overlap_tester,
                                                           all_dets, missing_detections);
    ...
    const matrix<double, 1, 3> cross_validate_object_detection_trainer(
        ...
        return cross_validate_object_detection_trainer(trainer, images, dets, ignore, folds, overlap_tester,
                                                       adjust_threshold);
    }
    ...
    const matrix<double, 1, 3> cross_validate_object_detection_trainer(
        ...
        return cross_validate_object_detection_trainer(trainer, images, truth_dets, ignore, folds, overlap_tester,
                                                       adjust_threshold);
    }
