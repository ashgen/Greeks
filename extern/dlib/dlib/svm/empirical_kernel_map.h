...
// ----------------------------------------------------------------------------------------
    ...
    const decision_function<kernel_type> convert_to_decision_function(
        ...
        DLIB_ASSERT(project_funct.out_vector_size() > 0 && is_vector(vect) &&
                    project_funct.out_vector_size() == vect.size() &&
        ...
        void load_impl(
            ...
            if (basis.size() == 0) {
                ...
                throw empirical_kernel_map_error(
                        "All basis_samples given to empirical_kernel_map::load() were zero vectors");
            }
            ...
            if (svd2(false, true, K, U, W, V)) {
                clear();
                throw empirical_kernel_map_error(
                        "While loading empirical_kernel_map with data, SVD failed to converge.");
            }
