    ...
    normalized_function<decision_function<radial_basis_kernel<matrix<double, 0, 1>>>> auto_train_rbf_classifier(
        ...
        auto cross_validation_score = [&](const double gamma, const double c1, const double c2) {
            ...
            if (be_verbose) {
                ...
                std::cout << "gamma: " << std::setw(11) << gamma << "  c1: " << std::setw(11) << c1 << "  c2: "
                          << std::setw(11) << c2 << "  cross validation accuracy: " << result << std::flush;
            }
        ...
        if (be_verbose) {
            ...
            std::cout << " best gamma: " << best_gamma << "   best c1: " << best_c1 << "    best c2: " << best_c2
                      << std::endl;
        }
