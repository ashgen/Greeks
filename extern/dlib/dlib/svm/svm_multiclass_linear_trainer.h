...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        multiclass_svm_problem(
                ...
                structural_svm_problem_threaded<matrix_type, std::vector<std::pair<unsigned long, typename matrix_type::type> > >(
                        num_threads),
                samples(samples_),
                labels(labels_),
        ...
        virtual void separation_oracle(
            ...
            for (unsigned long i = 0; i < distinct_labels.size(); ++i) {
                ...
                scalar_type temp = dot(mat(&current_solution(i * dims), dims - 1), samples[idx]) -
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        trained_function_type train(
            ...
            w_type weights;
            multiclass_svm_problem<w_type, sample_type, label_type> problem(all_samples, all_labels, df.labels, dims,
                                                                            num_threads);
            if (verbose)
                problem.be_verbose();
