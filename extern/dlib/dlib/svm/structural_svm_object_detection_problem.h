    ...
    private:
        ...
        virtual void get_truth_joint_feature_vector(
            ...
            for (unsigned long i = 0; i < mapped_rects.size(); ++i) {
                for (unsigned long j = i + 1; j < mapped_rects.size(); ++j) {
                    if (boxes_overlap(mapped_rects[i], mapped_rects[j])) {
                        ...
                        const double overlap_amount =
                                area_overlap / std::min(mapped_rects[i].area(), mapped_rects[j].area());
                        ...
                        sout
                                << "either need to relax the test_box_overlap object so it doesn't mark these rectangles as ";
                        ...
                        sout.str("");
                        sout << wrap_string(temp, 0, 0) << endl << endl;
            ...
            for (unsigned long i = 0; i < mapped_rects.size(); ++i) {
                ...
                if (area / total_area <= match_eps) {
                    ...
                    sout
                            << "so that it is capable of hitting this truth box, or adjust the offending truth rectangle so it ";
                    ...
                    sout
                            << "Additionally, the scan_fhog_pyramid scans a fixed aspect ratio box across the image when it ";
                    sout
                            << "searches for objects.  So if you are getting this error and you are using the scan_fhog_pyramid, ";
                    sout
                            << "it's very likely the problem is that your training dataset contains truth rectangles of widely ";
                    sout
                            << "varying aspect ratios.  The solution is to make sure your training boxes all have about the same aspect ratio. ";
                    ...
                    sout << "truth rect width/height: " << truth_object_detections[idx][i].get_rect().width() /
                                                           (double) truth_object_detections[idx][i].get_rect().height()
                         << endl;
                    ...
                    sout << "nearest detection template rect width/height: "
                         << mapped_rects[i].width() / (double) mapped_rects[i].height() << endl;
        ...
        virtual void separation_oracle(
            ...
            for (unsigned long i = 0; i < dets.size() && final_dets.size() < max_num_dets; ++i) {
                ...
                const std::pair<double, unsigned int> truth = find_best_match(truth_object_detections[idx],
                                                                              dets[i].second);
