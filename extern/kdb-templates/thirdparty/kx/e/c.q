...
/ constant time access to last by sym: select last.. by sym from t
ts:{1!$[count x;x last each(group x`sym)y;x]}

/vwap
vu:{select sym,price*size,size from x} 	/ then price%size
if[a~`v;img.trade:{v::su vu x};upd.trade:{v+:vu x}]
/u:{v+:select price:price wsum size,sum size by sym from x}
...
/u:{t,:(update sums price*size,sums size by sym from delete ex from x)pj ts[delete time from t]x`sym}
...
/u:{t,:select sym,time,price,0.0^wprice+price0*time-time0 from x lj ts[select sym,time0:time,price0:price,wprice from t]x`sym}
